...
/**
 * SettlementWarehouseReversal Model
 *
 * Fetch the Query to Insert into warehouse.settlement_warehouse for Reversed transaction
 */
class SettlementWarehouseReversal extends SettlementWarehouse {
 ...
 */
	private function __manageReversalsQuery($transData) {
		$sql = null;
 ...
 */
	public function createReversalsQuery($transIds,$date) {
		$transType = 'credit';
		...
		foreach ($data as $datum){
			if ($datum['CustomerTransaction']['original_transaction_id'] != null ) {
				$sqlQuery[] = $this->__manageReversalsQuery($datum);
			}
		}
