...
/**
 * Create row in merchant_ach_transactions,settlements and backend_transactions 
 * for each settlement_warehouse rows where settlement_sheduled_date = today 
 * and settlement_actual_date is null and customer_transaction.trans_type = 'debit'
 
 */
class SettlementCustomerDebitsStep extends Step {
	 ...
	 * @var string Format 'debit' or 'debit' 
	 */
	private $__transType;
	...
	public function __construct($date) {
		$this->__date = $date;
		$this->__transType = 'debit';
		$this->_query = array();
		$this->_idempotent = false;
		$this->_stepField = 'settlement_customer_debits';
	 ...
	 * 'settlement_customer_debits' to success
	 * 
	 * @param date $date Format 'Y-m-d'
	...
	/**
	 * Insert a row in 'merchant_ach_transactions','settlements' and 'backend_transactions '
	...
	public function executeInternal() {
		...
		if (!empty($settlementWarehouseData)) {
			$this->_query = $this->SettlementWarehouse->manageSettlementCustomerDebitCreditQuery(
				$settlementWarehouseData,$this->__date,'Debit');
		}
	...
	/**
	 *  Check if 'EodWorkflow'.'settlement_customer_debits' is updated to 'sucess'
	 * 
